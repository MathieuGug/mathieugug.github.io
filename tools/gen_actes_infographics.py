...
ACTES = {
    "I": {
        ...
        "chapters": [
            {"n": "1", "title": "Le cœur stochastique", "fact": "softmax · T · top-p · seed", "claim": "reproductibilité statistique remplace bit-à-bit", "schema": "S1.1 + S1.2"},
            {"n": "2", "title": "Modèles de raisonnement", "fact": "pivot o1 · RLVR · GRPO", "claim": "AIME 13 → 74 % single-sample en 6 mois", "schema": "E3 (capability×cost)"},
            {"n": "3", "title": "PRM : la couche notateur cachée", "fact": "économie souterraine 2-4 Md$", "claim": "reward hacking 99 % / verbalisation < 2 %", "schema": "E5 (PRM vs judge vs human)"},
            {"n": "4", "title": "Décode spéculative", "fact": "théorème Leviathan · 4 familles", "claim": "×3-6,5 throughput sans dégrader la qualité", "schema": "matrice frameworks"},
            {"n": "5", "title": "L'économie unitaire de l'inférence", "fact": "LLMflation ×1 000 en 4 ans", "claim": "pile 7 couches ×14 throughput sur B300", "schema": "E2 (pile 7 couches)"},
            {"n": "6", "title": "Bordure : world models", "fact": "JEPA · diffusion · autoregressif", "claim": "signal faible 18-36 mois (sauf computer use)", "schema": "thermomètre 6 verrous"},
        ],
        "callouts": [
    ...
    "II": {
        ...
        "chapters": [
            {"n": "7", "title": "Reason · Act · Observe", "fact": "harness · stop_reason · hooks", "claim": "POC sans harness défensif = 4 $/min en silence", "schema": "R1 boucle canonique + 3 variantes"},
            {"n": "8", "title": "Les outils (les mains de l'agent)", "fact": "function calling · code exec · sandbox", "claim": "execute_sql sans scoping = exfiltration en 3 tours", "schema": "renvois Ch.13 (MCP) + Ch.19 (sécu)"},
            {"n": "9", "title": "Mémoire agentique", "fact": "4 piliers × 6 opérations × 5 archis", "claim": "Letta · A-MEM · Zep · Mem0 · file-based", "schema": "R2 grille 4×5"},
            {"n": "10", "title": "Compaction & oubli stratégique", "fact": "5 familles · triangle fidélité×coût×oubli", "claim": "Lost in the Middle tient en 2026", "schema": "R3 triangle + SpAIware"},
            {"n": "11", "title": "Patterns canoniques & orchestration", "fact": "8 patterns · 4 régimes contrôle", "claim": "Klarna 67 % puis recul partiel (5 % cas charges)", "schema": "R4 patterns + arbre buy/build"},
        ],
        "callouts": [
    ...
    "III": {
        ...
        "chapters": [
            {"n": "12", "title": "MCP, le HTTP des agents", "fact": "trinité MCP × A2A × AG-UI", "claim": "97 M téléchargements/mois · 7 500 serveurs · LF déc 2025", "schema": "R6 layering protocoles"},
            {"n": "13", "title": "Sécurité MCP", "fact": "10 vecteurs × 10 patterns défensifs", "claim": "4 load-bearing : Sigstore · tagging · allowlist · HITL writes", "schema": "R7 matrice 10×10"},
            {"n": "14", "title": "Surfaces agentiques", "fact": "4 régimes d'accès Knight", "claim": "operator · collaborator · consultant · approver · observer", "schema": "R8 régimes + levels of autonomy"},
            {"n": "15", "title": "Computer use : le régime extrême", "fact": "observe · plan · ground · act · verify", "claim": "OSAgent dépasse baseline humain OSWorld oct 2025", "schema": "R9 boucle + cliff UI-CUBE"},
            {"n": "16", "title": "Analytics agentique (sectoriel régulé)", "fact": "3 surfaces GCP banque française tier 1", "claim": "75 jours avant AI Act art. 9-15 effectif (2 août 2026)", "schema": "R10 stack + matrice 6 réf"},
        ],
        "callouts": [
    ...
    "IV": {
        ...
        "chapters": [
            {"n": "17", "title": "Évaluer un agent (+ leaderboards)", "fact": "playbook gruyère 8 étapes", "claim": "4 vecteurs contamination · l'éval interne datée bat les scores publics", "schema": "R11 gruyère + R12 contamination"},
            {"n": "18", "title": "Observabilité & cognitive audit trail", "fact": "OTel GenAI semconv · 6 piliers", "claim": "réponse à AI Act art. 12-13-15 + RGPD art. 22", "schema": "R13 piliers + audit trail"},
            {"n": "19", "title": "Garde-fous · jailbreaking · sécu globale", "fact": "OWASP ASI Top 10 (déc 2025)", "claim": "Least agency · asymétrie attaque/défense · threat model unifié 2026", "schema": "E4 (textualisé §19.10)"},
            {"n": "20", "title": "Runtime managé & déploiement", "fact": "AgentCore · Vertex · AI Foundry", "claim": "buy/build : marges 45-80 % éclairent la décision", "schema": "E6 arbre décision"},
            {"n": "21", "title": "Mesurer le ROI (paradoxe agentique)", "fact": "J-curve Brynjolfsson · 5 frameworks", "claim": "token → tâche → processus → outcome (Klarna)", "schema": "R16 (J-curve × LLMflation)"},
            {"n": "22", "title": "Externalité énergétique : IA frugale", "fact": "3 scopes × 3 phases (GHG)", "claim": "Jevons +48 % Google malgré -10×/req · 3 trajectoires 2030", "schema": "R17 trajectoires + sankey eau"},
            {"n": "23", "title": "Gouvernance : AI Act, banque, unlearning", "fact": "AI Act art. 9-15 effectif 2 août 2026", "claim": "6 référentiels convergents · 18 mois · 4 rôles pivot", "schema": "R18 calendrier unifié"},
        ],
        "callouts": [
...
def make_svg(acte_key, data):
    ...
    for i, ch in enumerate(chapters):
        ...
        title_lines = wrap_text(ch["title"], 22)
        ...
        fact_lines = wrap_text(ch["fact"], 26)
        ...
        claim_lines = wrap_text(ch["claim"], 28)
        ...
        schema_lines = wrap_text(ch["schema"], 30)
