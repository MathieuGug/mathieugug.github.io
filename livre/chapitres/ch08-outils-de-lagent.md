...
Côté gouvernance, c'est tout sauf transparent. Un serveur MCP peut être tiers (`@modelcontextprotocol/server-github`), interne (votre propre serveur exposant le data warehouse), ou hostile (le **tool poisoning** OWASP ASI02[^5]). La dyade [Ch. 15](ch15-mcp-plateforme.md) (MCP plateforme) / [Ch. 16](ch16-mcp-securite.md) (sécurité MCP) traite ce sujet en profondeur — retenir ici : ==un tool MCP n'est pas neutre, c'est un tool dont la description et le schéma viennent d'un tiers==. Sigstore + hash pinning, allowlist namespace, HITL writes : les quatre patterns load-bearing du [Ch. 16](ch16-mcp-securite.md) s'appliquent ici.
