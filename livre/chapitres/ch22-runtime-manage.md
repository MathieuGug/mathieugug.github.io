...
## 22.1 Pourquoi 2026 est l'année des runtimes managés

### 22.1.1 Le déplacement
...
### 22.1.2 L'autre raison — économique
...
## 22.2 Trois couches à ne pas confondre
...
### 22.2.1 Couche 1 — ADK (Agent Development Kit)
...
### 22.2.2 Couche 2 — Runtime (exécution)
...
### 22.2.3 Couche 3 — Services de plateforme

Là où les hyperscalers se distinguent vraiment, et où la complexité se cache. **AgentCore propose six briques distinctes**, chacune consommable indépendamment[^12]. C'est exactement le sujet de la section §22.3.
...
## 22.3 Les six briques de plateforme — anatomie AgentCore
...
### 22.3.1 Memory — court terme + long terme
...
### 22.3.2 Identity — l'agent au nom de l'utilisateur
...
Conséquence sécurité : sans identité déléguée, ==une injection indirecte dans un email lu déclenche une action avec les droits *admin* du service==. C'est exactement le scénario EchoLeak (cf. [Ch. 21](ch21-gardefous-securite-globale.md) §21.5.1) — la mitigation structurelle est *on-behalf-of*, pas un classifier de plus.

### 22.3.3 Gateway — outils MCP
...
C'est le layer où la matrice 10×10 du [Ch. 16](ch16-mcp-securite.md) s'instancie. Le service managé absorbe une partie du plumbing (auth, retries, *circuit breaking*, observabilité), pas la **politique de sécurité** — qui reste à votre charge. Sigstore + hash pinning + tool tagging + HITL writes, c'est vous qui décidez.

### 22.3.4 Observability — traces OTel natives

Tracing OpenTelemetry, vue unifiée des appels et des erreurs. AgentCore Observability émet des spans `gen_ai.*` conformes à la spec v1.37+ (cf. [Ch. 20](ch20-observabilite-cognitive-audit-trail.md)). Vertex et Foundry idem. La force du managé : ==le tracing est natif, pas optionnel==. La limite : si vous voulez le crosser avec votre stack APM existante (Dynatrace, Datadog), il faut exporter via OTLP — ce qui est possible mais à configurer.

Le [Ch. 20](ch20-observabilite-cognitive-audit-trail.md) §20.8 a posé l'arbitrage incumbents APM × challengers AI-native. Avec un runtime managé, vous avez **par défaut** un backend AI-native ; pour la couche infra/topologie, vous restez sur votre stack incumbent.

### 22.3.5 Code Interpreter — sandbox Python/JS/TS jusqu'à 8 h
...
Navigateur distant : l'agent navigue le web *comme un humain*, sans pourrir la machine locale. Pour les agents computer-use ([Ch. 17](ch17-computer-use.md)), c'est le pendant managé du *headless browser* qu'on devait gérer soi-même. Le service offre une session isolée, un cookie store éphémère, des screenshots, et un protocole d'interaction (clicks, types, scrolls).

==Surface d'attaque inédite==, intimement liée à [Ch. 17](ch17-computer-use.md) : Visual Prompt Injection (VPI). Le browser tool managé sandbox la *navigation*, pas le *rendu*. Une page web hostile peut toujours injecter via une image rendue. Mitigation : OCR + classifier (cf. [Ch. 21](ch21-gardefous-securite-globale.md) surface (vi)).

> [!INFO] Voir [Ch. 20 — Observabilité](ch20-observabilite-cognitive-audit-trail.md) · [Ch. 21 — Sécurité](ch21-gardefous-securite-globale.md) · [Ch. 9](ch09-memoire-agentique.md)-[10](ch10-compaction.md) — Mémoire/Compaction
> Chaque brique de plateforme s'instancie dans une discipline déjà traitée. Memory ⇒ [Ch. 9](ch09-memoire-agentique.md)-[10](ch10-compaction.md). Identity + Gateway ⇒ [Ch. 21](ch21-gardefous-securite-globale.md) surfaces (iv) et (vi). Observability ⇒ [Ch. 20](ch20-observabilite-cognitive-audit-trail.md). Code Interpreter + Browser Tool ⇒ [Ch. 21](ch21-gardefous-securite-globale.md) sandboxing + [Ch. 17](ch17-computer-use.md) computer use. La valeur ajoutée du runtime managé n'est pas de *réinventer* ces disciplines — c'est de **fournir une implémentation par défaut** que vous gouvernez par configuration, pas par code.

---

## 22.4 Cartographie hyperscaler — qui propose quoi
...
### 22.4.1 AWS Bedrock AgentCore (GA 13 octobre 2025)

Le plus modulaire avec ses six briques indépendantes (§22.3). Pricing **consumption-based pur** : *« charge only for what's consumed »*[^12]. Sessions jusqu'à 8 h en Runtime. Intégrations natives Bedrock (modèles Claude, Llama, Titan, Mistral, Cohere via API unifiée), S3, DynamoDB, Lambda, EventBridge. ==Choix par défaut pour les organisations déjà AWS-natives== avec une équipe SRE/DevOps qui sait composer les briques.
...
### 22.4.2 Google Vertex AI Agent Engine
...
### 22.4.3 Azure AI Foundry Agent Service (GA mai 2025)
...
### 22.4.4 Claude Managed Agents (public beta avril 2026)
...
### 22.4.6 Synthèse — choisir, pas comparer
...
## 22.5 Le chemin de productionisation
...
### 22.5.1 Prototyper dans Claude Code (ou équivalent)
...
### 22.5.2 Extraire la logique dans le SDK
...
### 22.5.3 Choisir une cible de déploiement
...
## 22.6 Sécurité gruyère — trois couches qui combinent

Le [Ch. 21](ch21-gardefous-securite-globale.md) a posé les **cinq couches** de la défense en profondeur agentique. Côté runtime, le manuel d'Anthropic[^13] décline une version pragmatique pour un agent bash-driven : **trois couches** spécifiques au runtime, qui s'imbriquent dans le stack à cinq couches du [Ch. 21](ch21-gardefous-securite-globale.md).

![Le gruyère suisse à trois couches — sécurité d'un agent bash-driven|1300](../../agent-sdk/images/20260518-09-securite-couches.svg)

### 22.6.1 Couche 1 — Alignement modèle

Anthropic, OpenAI, Google investissent massivement sur l'alignement du modèle lui-même : il refuse les actions destructrices sans confirmation, il évite les prompt injections évidentes, il ne s'auto-exfiltre pas. Publication Anthropic sur le reward hacking — un modèle bien aligné ne triche pas pour atteindre son objectif. ==C'est la première barrière, mais elle ne suffit jamais== (cf. [Ch. 21](ch21-gardefous-securite-globale.md) §21.7.1 — Layer 1 training-time alignment).

### 22.6.2 Couche 2 — Harness controls
...
==Aucune des trois couches n'est suffisante seule — c'est la composition qui rend l'ensemble robuste.== C'est exactement le motif de défense en profondeur du [Ch. 21](ch21-gardefous-securite-globale.md) §21.10 appliqué au runtime.

### 22.6.4 RBAC — côté infra, pas côté prompt
...
C'est précisément ce que résout *Identity / on-behalf-of* (§22.3.2) au niveau du runtime managé : l'agent porte les *droits de l'utilisateur* via OAuth/OIDC, pas une clé service avec droits admin. La fenêtre d'erreur s'effondre.
...
## 22.7 Pricing consumption-based — anatomie de la facture

### 22.7.1 Ce qui change vs ECS / Lambda
...
### 22.7.2 Les six postes de coût d'un agent managé en production
...
6. **Observability** — facturé sur volume de spans + rétention. **Le poste le plus mal anticipé** (cf. [Ch. 20](ch20-observabilite-cognitive-audit-trail.md) §20.8.1 sur le piège per-LLM-span).

### 22.7.3 Les pièges de signature
...
- **Egress data** — sortir les données du cloud (vers backup, audit externe, ou autre cloud) coûte parfois plus que le compute. Pour un cognitive audit trail à rétention 5 ans (cf. [Ch. 20](ch20-observabilite-cognitive-audit-trail.md) §20.6.1), c'est un poste à dimensionner.
...
## 22.8 Anti-lock-in — code-first + protocoles ouverts
...
### 22.8.1 Trois protocoles transverses
...
### 22.8.3 Le sweet spot anti-lock-in
...
> [!INFO] Voir [Ch. 15 — MCP plateforme](ch15-mcp-plateforme.md) · [Ch. 11 — Patterns d'orchestration](ch11-patterns-orchestration.md)
> Le [Ch. 15](ch15-mcp-plateforme.md) détaille MCP comme *« HTTP des agents »* et l'effet de réseau qui l'a installé en standard de facto. Le [Ch. 11](ch11-patterns-orchestration.md) §11.5.2 a introduit la trilogie ADK / runtime / plateforme et l'arbre de décision buy/build. Ici, le **deep-dive runtime**.

---

## 22.9 Décider — quand utiliser un runtime managé ?
...
**Q5 — Compliance régulée (banque, santé, public) ?** Vérifier que le runtime managé offre Assured Workloads / S3NS / SecNumCloud / souveraineté de données alignés sur votre threat model (cf. [Ch. 18](ch18-analytics-agentique-banque.md) et [Ch. 25](ch25-gouvernance-ai-act.md)). Sinon, déploiement hybride (runtime self-hosted dans région souveraine + modèle via API régionale).

### 22.9.1 Signaux de migration vers managé
...
- **Le métier vous demande des évals continues, du replay, des audits.** Le graphe déclaratif + checkpointing devient quasi-obligatoire ; un agent autonome non-instrumenté ne passera pas la revue (cf. [Ch. 20](ch20-observabilite-cognitive-audit-trail.md) cognitive audit trail).

### 22.9.2 Signaux de migration *depuis* managé
...
## 22.10 Récap chapitre — trois couches, une discipline
...
4. **Configurer les six briques de plateforme explicitement.** Memory avec politique d'expiration (RGPD art. 17 — [Ch. 10](ch10-compaction.md)). Identity en *on-behalf-of* (sécurité — [Ch. 21](ch21-gardefous-securite-globale.md)). Gateway en MCP avec allowlist namespace + Sigstore ([Ch. 16](ch16-mcp-securite.md)). Observability en OTel natif ([Ch. 20](ch20-observabilite-cognitive-audit-trail.md)). Code Interpreter et Browser Tool sandboxés.
