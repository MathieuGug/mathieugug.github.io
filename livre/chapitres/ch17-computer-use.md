...
### 17.1.2 Quatre lignées chronologiques
...
Microsoft entre dans la danse en février 2026 en exposant des **computer-using agents** dans Copilot Studio avec choix entre OpenAI CUA et Claude Sonnet 4.5[^23]. C'est la première convergence enterprise : un client final ne choisit plus le vendeur de modèle, il choisit l'orchestrateur — l'orchestrateur choisit le modèle selon le cas d'usage. Pattern qui se retrouve dans Vertex Agent Engine ([Ch. 18](ch18-analytics-agentique-banque.md)).

==L'inflexion 2026 n'est plus celle des capacités brutes mais celle de l'économie unitaire.== La conversation produit se déplace : non plus *que peut faire un agent*, mais *combien coûte une tâche réussie de bout en bout, et avec quelle variance*. Les sections §17.8 et §17.9 développent ces deux questions.

---

## 17.2 Architecture canonique — la boucle à cinq phases
...
### 17.2.5 Vérifier — le cinquième nœud qui change tout
...
## 17.3 Trois architectures concurrentes
...
### 17.3.1 Architecture (i) — vision pure
...
### 17.3.2 Architecture (ii) — vision + parseur dédié
...
### 17.3.3 Architecture (iii) — agent intégré perception-action
...
## 17.4 Paysage concurrentiel — quatre couches

Le paysage début 2026 se structure en quatre couches. Le [Ch. 13](ch13-surfaces-agentiques.md) a déjà nommé les acteurs côté business (Operator, Computer Use, Devin, Sierra, Harvey, Agentforce) ; ici, la **stack technique**.
...
### 17.4.2 Couche orchestration et harness
...
### 17.4.4 Couche enterprise

**Copilot Studio**, **Salesforce Agentforce**, et les grandes ESN (Accenture, Capgemini, Deloitte) emballent ces primitives dans des offres conformes (gouvernance, audit, isolation). C'est la couche qui matérialise la conformité DORA / AI Act que le [Ch. 25](ch25-gouvernance-ai-act.md) cadrera génériquement et que le [Ch. 18](ch18-analytics-agentique-banque.md) instancie sur la banque française.

### 17.4.5 Souveraineté — la voie UI-TARS
...
## 17.5 Benchmarks — savoir ce qu'on mesure
...
### 17.5.1 La trajectoire OSWorld — convexe mais piégée
...
### 17.5.2 Le verdict UI-CUBE — le cliff enterprise
...
### 17.5.3 Robustesse à l'attaque — chiffres alarmants

Sur la robustesse, les chiffres sont alarmants. **VPI-Bench** mesure la susceptibilité à l'injection de prompt visuelle — un attaquant qui cache une instruction dans un élément visuel rendu (banner, infobulle, image manipulée). Le résultat : ==**51 % de succès** contre les CUA et **jusqu'à 100 %** contre les BUA sur certaines plateformes==[^8]. **AgentDojo** enfonce le clou : sur 97 tâches × 629 cas de test, les défenses existantes améliorent peu la situation[^34]. La §17.7 développe les patterns d'attaque, et §17.10 fait le pont avec le threat model unifié E4 du [Ch. 21](ch21-gardefous-securite-globale.md).

> [!INFO] Voir [Ch. 19 — Évaluation et benchmarks](ch19-evaluation-benchmarks.md)
> Le [Ch. 19](ch19-evaluation-benchmarks.md) a posé la grille générique : 4 vecteurs de fuite des benchmarks publics (chevauchement temporel / version-tag / harness gaming / prompt leakage), le 2×2 contrôlé × ponctuel pour acheteurs, le playbook gruyère 8 étapes. Ici, instanciation sur OSWorld et UI-CUBE — **pas** de redémonstration générique. La frontière éditoriale : [Ch. 19](ch19-evaluation-benchmarks.md) = grille d'achat et démolition leaderboards ; ici, lecture honnête des chiffres CUA *en supposant connue* la grille.

---

## 17.6 Cas d'usage et frameworks
...
### 17.6.1 Les déploiements nominatifs
...
### 17.6.2 E-commerce et booking — le terrain le moins convaincant
...
### 17.6.3 Back-office et ITSM — le sweet spot
...
### 17.6.4 Customer-service voix — exclus
...
### 17.6.5 Frameworks — trois approches
...
## 17.7 Surface d'attaque qualitativement inédite
...
### 17.7.1 VPI — l'injection visuelle qui n'a pas d'équivalent défensif
...
VPI-Bench mesure 306 cas sur 5 plateformes : **succès jusqu'à 51 % contre les CUA et 100 % contre les BUA** sur certaines surfaces[^8]. Les défenses existantes (filtrage texte, sandboxes DOM) sont structurellement inopérantes — ==elles ne couvrent pas le canal pixel==. C'est précisément ce que le [Ch. 16](ch16-mcp-securite.md) ne pouvait pas couvrir : la matrice MCP 10×10 traite les vecteurs textuels (description empoisonnée, prompt injection cross-document, cross-server confusion, OAuth+supply chain) ; le canal pixel d'un screenshot interprété par un VLM est une **verticale orthogonale**.

> [!IMPORTANT] La verticale VPI s'ajoute à E4 ([Ch. 21](ch21-gardefous-securite-globale.md))
> Le threat model unifié E4 du [Ch. 21](ch21-gardefous-securite-globale.md) agrège six axes : modèle / prompt / mémoire / outil / protocole / **surface**. ==Le canal VPI est ce qui peuple l'axe « surface »== — il n'apparaît pas dans la matrice MCP du [Ch. 16](ch16-mcp-securite.md) parce qu'il ne passe pas par le tuyau JSON-RPC ; il passe par le canal d'observation du modèle. La défense en profondeur agentique 2026 doit donc additionner : signature Sigstore + hash pinning (couche tool, [Ch. 16](ch16-mcp-securite.md)), classifieurs de prompt injection visuelle (couche surface, ici), et human-in-the-loop sur les actions irréversibles (couche action, [Ch. 13](ch13-surfaces-agentiques.md) §13.6.3).

### 17.7.2 Dérive d'exécution
...
### 17.7.4 CVE-2025-55322 — quand le control plane devient le vecteur
...
### 17.7.5 Reliability — runs divergents

Côté fiabilité, le problème est plus prosaïque mais aussi plus universel. L'étude *On the Reliability of Computer Use Agents* (avril 2026) montre qu'un même agent, exécuté à plusieurs reprises sur la même tâche, produit des trajectoires divergentes liées à trois sources : stochasticité du modèle, ambiguïté des instructions, variabilité de la planification[^42]. ==Pour la production, cela signifie qu'un test unique ne suffit pas — il faut des *runs* répétés, idéalement avec mécanismes de guidance par exécutions précédentes== (in-context examples). C'est la justification opérationnelle de la métrique *repro rate* que le [Ch. 18](ch18-analytics-agentique-banque.md) §18.9.2 réinjecte dans la grille d'évaluation data-spécifique.

---

## 17.8 La latence — le coût caché plus que la précision
...
## 17.9 Marché 2026-2030 et inflexions
...
### 17.9.1 Les trois prédictions Gartner à dater
...
### 17.9.2 Trois lignes de fracture
...
**La troisième est régulatoire.** Gartner prédit que des lois IA fragmentées couvriront la moitié de l'économie mondiale d'ici 2027, induisant ~5 G$ de dépenses de conformité[^11]. L'EU AI Act, l'AI Office et les *deployer obligations* pour les systèmes à haut risque structureront le déploiement européen. Le [Ch. 18](ch18-analytics-agentique-banque.md) instancie sur la banque française tier 1 ; le [Ch. 25](ch25-gouvernance-ai-act.md) généralise.

### 17.9.3 Trois inflexions à dater 18-24 mois
...
> L'inflexion (ii) — standard d'observabilité agent — est précisément ce que le [Ch. 20](ch20-observabilite-cognitive-audit-trail.md) documente comme la convergence 2026 autour d'OpenTelemetry GenAI Semantic Conventions et du *cognitive audit trail*. Les traces UI (screenshots, coordonnées d'action, états DOM) sont la prochaine extension naturelle des semconv — front actif fin 2026, mentionné comme `gen_ai.computer_use.*` dans les early draft WG.
...
## 17.10 Récap chapitre — Boucle, archis, surface, économie
...
3. **La surface d'attaque qualitativement inédite** — VPI (51 % CUA / 100 % BUA) + CVE-2025-55322 control plane. Ces deux verticales s'ajoutent au threat model E4 du [Ch. 21](ch21-gardefous-securite-globale.md) et ne sont pas couvertes par la matrice MCP du [Ch. 16](ch16-mcp-securite.md).
...
> **Piège 1 — RFP au score OSWorld brut.** Sans inspecter (a) la version du benchmark (Verified ou non), (b) si le système utilise code execution comme raccourci, (c) SELF vs 3RD party — un score OSWorld brut n'est pas un signal d'achat. Le [Ch. 19](ch19-evaluation-benchmarks.md) a déjà nommé le pattern ; ici, il s'instancie.
> **Piège 2 — POC sans test reliability répété.** Une même tâche rejouée dix fois produit dix trajectoires divergentes. Tester une fois est un POC ; tester N fois avec mesure de variance est une éval. La métrique *repro rate* ([Ch. 18](ch18-analytics-agentique-banque.md) §18.9.2) doit figurer dans le gating de release.
> **Piège 3 — Skipper le control plane audit.** Tout endpoint HTTP qui pilote la VM agent est un endpoint d'attaque. CVE-2025-55322 sur OmniParser est l'exemple canonique ; il s'en publiera d'autres. La revue sécurité doit inclure le control plane ET le canal pixel — la matrice MCP du [Ch. 16](ch16-mcp-securite.md) ne suffit pas.

Le [Ch. 18](ch18-analytics-agentique-banque.md) ferme l'Acte sur l'instanciation sectorielle — trois surfaces agentiques GCP, banque française tier 1, pivot sémantique, régulation 2 août 2026.
