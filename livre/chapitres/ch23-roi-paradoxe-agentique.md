...
## 23.1 Le paradoxe ROI 2026

### 23.1.1 Quatre chiffres et un diagnostic
...
### 23.1.2 La méthode comptable classique est l'angle mort
...
> [!INFO] Voir [Ch. 5 — Économie de l'inférence](ch05-economie-inference.md) · [Ch. 19 — Évaluer un agent (et débunker les leaderboards)](ch19-evaluation-benchmarks.md) · [Ch. 24 — Externalité énergétique : IA frugale](ch24-ia-frugale.md) · [Ch. 25 — Gouvernance : AI Act, banque, machine unlearning](ch25-gouvernance-ai-act.md)
> La **face métier** du triptyque tarifaire qui traverse l'Acte IV. Le [Ch. 5](ch05-economie-inference.md) a posé la physique du coût par token (sept couches d'optimisation, LLMflation ×1000 entre 2022 et 2025) ; le [Ch. 19](ch19-evaluation-benchmarks.md) a posé la mesure de la qualité ; ici, la valeur métier ; le [Ch. 24](ch24-ia-frugale.md) mesurera l'externalité énergétique. Trois lectures complémentaires de la même facture. Le [Ch. 25](ch25-gouvernance-ai-act.md) consommera la checklist en sept questions §23.9 comme entrée des *gates* de gouvernance.

---

## 23.2 Pourquoi les méthodes IT classiques calent

### 23.2.1 Première rupture — le coût marginal tend vers zéro, le coût total explose
...
### 23.2.2 Deuxième rupture — les bénéfices se diluent sur un workflow
...
C'est l'aspect le plus mal compris du débat. Le sponsor IA qui dit *« on a déployé Copilot, on devrait voir un gain de productivité »* mesure la mauvaise chose. Le gain de productivité existe au niveau atomique (la tâche est faite plus vite, c'est documenté empiriquement — voir §23.8) mais il s'évapore au niveau organisationnel **si rien n'a été redesigné**. La J-curve commence ici.

### 23.2.3 Troisième rupture — la J-curve de Brynjolfsson, Rock & Syverson
...
### 23.3.2 McKinsey — l'indicateur d'aboutissement
...
### 23.3.4 MIT NANDA — le diagnostic *learning gap*
...
### 23.3.5 Forrester TEI — la méthode de chiffrage
...
## 23.4 Une grille en cinq axes pour cartographier la valeur
...
### 23.4.1 Les cinq axes et leurs questions de cadrage
...
### 23.4.2 Le choix de l'axe détermine le KPI métier
...
### 23.4.3 Mapping bidirectionnel avec Cigref
...
### 23.4.4 Les trois angles morts honnêtes
...
- **La valorisation des données propriétaires.** Un agent qui exploite mieux la base client interne crée une valeur d'option (capacité à monétiser, à différencier, à pivoter sur un segment). Aucune des cinq dimensions ne la chiffre proprement. Traitement : la déclarer en *flexibilité* dans un Forrester TEI (§23.6.1 méthode 8), pas en gain direct.
...
## 23.5 Hard Savings vs Soft Savings — la vraie ligne de partage
...
- **Vitesse 35 % Hard / 65 % Soft** — le gain de cycle time est mesurable, mais la conversion en valeur P&L exige une preuve de réallocation (cf. §23.5.3).
...
### 23.5.2 Les deux écueils symétriques
...
### 23.5.3 Règle de validation Hard — la preuve de réallocation
...
### 23.5.4 Règle de validation Soft — la convergence à trois indicateurs
...
## 23.6 La boîte à outils méthodologique

Une fois l'axe identifié (§23.4) et la nature Hard/Soft tranchée (§23.5), il reste à choisir une **méthode de calcul**. Huit méthodes principales suffisent à couvrir l'essentiel des cas d'usage IA générative et agentique.

### 23.6.1 Huit méthodes en une table
...
### 23.6.2 Arbre de décision à trois questions
...
### 23.6.3 Trois pièges pratiques à connaître
...
## 23.7 Le paradoxe agentique — l'unité de mesure se déplace
...
> [!INFO] Voir le schéma R16 (double-page éco) en §23.2.3
...
### 23.7.1 La stack à quatre niveaux
...
2. **Tâche** — une étape atomique réalisée par l'agent (résoudre un ticket, générer un brouillon, requêter une base). Métrique : coût par tâche, taux de succès, qualité. C'est l'unité où la productivité GenAI se mesure au mieux dans la littérature empirique (§23.8).
...
### 23.7.2 Klarna — le cas d'école de la remontée échouée
...
### 23.7.4 Le diagnostic intra-stack
...
| **Tâche** | Taux de succès stagne à 60-70 % | Tools mal scopés, grader pas calibré, instructions ambiguës | Refactor des tools, calibration LLM-as-judge (voir [Ch. 19](ch19-evaluation-benchmarks.md)), simplification prompts |
| **Processus** | Taux d'autonomie bloqué autour de 40 % | Escalades non instrumentées, agents en boucle, mémoire insuffisante | Trajectoire monitoring (voir [Ch. 20](ch20-observabilite-cognitive-audit-trail.md)), mémoire architecturée (voir [Ch. 9](ch09-memoire-agentique.md)), patterns orchestration (voir [Ch. 11](ch11-patterns-orchestration.md)) |
...
## 23.8 Ce que disent vraiment les études empiriques
...
### 23.8.1 Brynjolfsson — support client, +14 % moyen / +34 % novices
...
### 23.8.2 Copilot — devs, +55,8 % en tâche / +26 % en PR/semaine
...
### 23.8.3 Jagged Frontier — consultants, ±40 % in / -19 pts out
...
### 23.8.4 METR — devs OSS très expérimentés, -19 %
...
## 23.9 Pièges à éviter et checklist en sept questions

### 23.9.1 Cinq pièges à éviter
...
### 23.9.2 Sept questions de signature
...
> Ces sept questions sont les entrées des *gates* de gouvernance du [Ch. 25](ch25-gouvernance-ai-act.md). Un projet qui n'a pas répondu aux sept ne devrait pas franchir la première ligne de défense (équipes opérationnelles). Le rôle pivot *Sponsor IA* (cf. [Ch. 25](ch25-gouvernance-ai-act.md) §25.5) est porteur de cette discipline.

---

## 23.10 Récap chapitre — la grille minimale du décideur IA 2026
...
Le [Ch. 24](ch24-ia-frugale.md) (frugalité énergétique) ferme le triptyque tarifaire : prix par token qui baisse ([Ch. 5](ch05-economie-inference.md)), valeur par outcome qui monte lentement (ici), externalité énergétique qui n'est jamais sur la facture ([Ch. 24](ch24-ia-frugale.md)). Trois lectures complémentaires d'une même facture — et la facture environnementale est, comme la facture J-curve, retardée et invisible jusqu'au moment où elle ne l'est plus.
