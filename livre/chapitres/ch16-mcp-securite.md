...
> - **La matrice défensive tient en dix patterns, dont quatre pivots.** Signature Sigstore + hash pinning (couvre familles A et D). Tool tagging au runtime (couvre A et C). Allowlist namespace par serveur (couvre C). Human-in-the-loop sur les `write` tools (couvre B). Les six autres patterns — OTel audit log standardisé, content provenance, rate limiting, refresh token rotation, scope-narrow OAuth, sandbox processus — réduisent la *blast radius* mais ne préviennent pas l'attaque initiale.
...
> Le pattern *confused deputy* (Hardy 1988) décrit un programme privilégié qui agit pour le compte d'un appelant moins privilégié, sans préserver la séparation d'autorité. Le LLM en MCP est un confused deputy *intrinsèque* : il dispose des privilèges de l'utilisateur sur tous les outils connectés, et il prend ses décisions en lisant du texte qui peut venir de l'utilisateur **ou** d'un document tiers. ==Distinguer les deux n'est pas une feature optionnelle de MCP — c'est la condition pour que la sécurité ne soit pas une fiction.== Les patterns pivots (§16.6) sont précisément des mécanismes pour rétablir cette distinction à l'extérieur du LLM, puisqu'on ne peut pas l'imposer à l'intérieur.
...
### 16.3.3 La défense pivot — HITL sur les `write` tools
...
## 16.6 La matrice défensive — dix patterns, quatre pivots
...
### 16.6.2 Les quatre patterns pivots
...
> ==Le calendrier réglementaire (AI Act art. 15, 2 août 2026) et le calendrier technique (spec MCP v2 Sigstore, automne 2026) convergent à ±60 jours.== Pour une équipe qui déploie un agent MCP en production en 2026, l'adoption simultanée des quatre patterns pivots (Sigstore + hash pinning, tool tagging, allowlist namespace, HITL writes) coche en un seul effort : (1) la conformité technique au futur MCP v2 ; (2) la documentation de robustesse exigée par l'art. 15 ; (3) la traçabilité d'audit pour le RSSI ; (4) les exigences sectorielles EBA/ACPR pour la banque française (cf. [Ch. 18](ch18-analytics-agentique-banque.md) §18.11). Quatre fronts, un seul investissement de plomberie. Cf. [Ch. 25](ch25-gouvernance-ai-act.md) pour le calendrier réglementaire complet et le rôle DPO/RSSI/Sponsor.
...
==**À retenir** : dix vecteurs en ligne, dix patterns en colonne, quatre patterns pivots marqués d'un cartouche.== La matrice n'est pas un poster décoratif — c'est une grille d'audit que le RSSI doit pouvoir cocher ligne par ligne avant de signer la mise en production d'un agent MCP. Couvrir les quatre patterns pivots élimine les familles A, B, C ; couvrir trois des six patterns secondaires réduit la *blast radius* sur la famille D au prix acceptable d'un peu de plomberie. Sans cette grille, le déploiement MCP en 2026 est un pari à 18 mois sur le fait qu'aucune des dix attaques connues ne croisera votre périmètre — pari raisonnable pour un prototype, inacceptable pour de la production.

---

> [!WARNING] Piège classique
> Traiter MCP comme un problème de **runtime**. Sandboxes, garde-fous *post-hoc*, *humans in the loop* indifférenciés sur tous les appels : c'est la posture défensive que l'industrie a tenue dix-huit mois et qui ne tient plus. ==Le contrat de confiance se construit *avant* l'exécution, pas après par audit.== Sigstore + hash pinning + tool tagging + allowlist namespace + HITL ciblé sur les `write` tools — les quatre patterns pivots — déplacent la défense de l'install à l'usage, du humain au protocole. Les équipes qui restent en posture runtime patching paieront, à 24 mois, un coût d'incident dont le post-mortem GitHub README-to-Tool de juin 2025 n'est qu'un avant-goût. Le pivot infrastructure (spec automne 2026) n'attendra pas que tout le monde soit prêt.
