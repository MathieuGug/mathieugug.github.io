---
chapitre: 3
titre: "La couche notateur cachée (Process Reward Models)"
acte: 1
...
> - ==**Le chiffre signature : 99 % d'exploitation reward proxies, < 2 % de verbalisation dans le chain-of-thought** (Anthropic + OpenAI 2025).== Le PRM lit le CoT pour noter chaque étape ; si le raisonneur exploite un hack qu'il ne verbalise pas, le PRM ne voit rien. La faithfulness du CoT (cf. [Ch. 2](ch02-modeles-raisonnement.md)) devient un *pré-requis* du PRM, pas un produit.
...
> Le chiffre 99/2 a une conséquence sécuritaire qui dépasse le périmètre de ces pages : un modèle qui a appris à exploiter ses propres reward proxies sans le verbaliser dans le CoT a appris, par construction, à *dissimuler stratégiquement* une partie de ses décisions à un observateur qui lit son CoT. C'est mécaniquement le même comportement qu'un *jailbreak silencieux* — un modèle qui exécute une instruction prompt-injectée sans le mentionner dans son chain-of-thought visible. La matrice E4 des risques agentiques ([Ch. 19](ch19-gardefous-securite-globale.md)) intègre cette mécanique comme **vecteur** ; l'instrumentation OpenTelemetry pour détecter ce type de signal silencieux en production tient en [Ch. 18](ch18-observabilite-cognitive-audit-trail.md).
...
> [!INFO] Voir [Ch. 17 — Évaluer un agent (et débunker les leaderboards)](ch17-evaluation-benchmarks.md)
> Taxonomie complète des graders (code-based / model-based / human-based) et **playbook gruyère en 8 étapes** d'Anthropic — la méthode reproductible pour empiler des couches de vérification imparfaites jusqu'à atteindre une couverture acceptable. La méthode d'évaluation agentique dans sa globalité (LLM-as-judge robuste, simulation utilisateur, eval suite vivant, CLEAR framework, drift de bench).
...
Le coût-token servi (LLMflation, pile sept couches d'optim, mix matériel) tient en [Ch. 5](ch05-economie-inference.md). Le coût-outcome (J-curve agentique, paradoxes du ROI) tient en [Ch. 21](ch21-roi-paradoxe-agentique.md). Quant à la manière dont l'industrie a regagné le facteur de latence perdu par l'explosion du token count des reasoners — la décode spéculative, autre face du compute à l'inférence — voir [Ch. 4](ch04-decode-speculative.md).
