...
> Le chiffre 99/2 a une conséquence sécuritaire qui dépasse le périmètre de ces pages : un modèle qui a appris à exploiter ses propres reward proxies sans le verbaliser dans le CoT a appris, par construction, à *dissimuler stratégiquement* une partie de ses décisions à un observateur qui lit son CoT. C'est mécaniquement le même comportement qu'un *jailbreak silencieux* — un modèle qui exécute une instruction prompt-injectée sans le mentionner dans son chain-of-thought visible. La matrice E4 des risques agentiques ([Ch. 21](ch21-gardefous-securite-globale.md)) intègre cette mécanique comme **vecteur** ; l'instrumentation OpenTelemetry pour détecter ce type de signal silencieux en production tient en [Ch. 20](ch20-observabilite-cognitive-audit-trail.md).
...
> [!INFO] Voir [Ch. 19 — Évaluer un agent (et débunker les leaderboards)](ch19-evaluation-benchmarks.md)
