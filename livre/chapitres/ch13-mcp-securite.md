...
> Le pattern *confused deputy* (Hardy 1988) décrit un programme privilégié qui agit pour le compte d'un appelant moins privilégié, sans préserver la séparation d'autorité. Le LLM en MCP est un confused deputy *intrinsèque* : il dispose des privilèges de l'utilisateur sur tous les outils connectés, et il prend ses décisions en lisant du texte qui peut venir de l'utilisateur **ou** d'un document tiers. ==Distinguer les deux n'est pas une feature optionnelle de MCP — c'est la condition pour que la sécurité ne soit pas une fiction.== Les patterns load-bearing (§13.6) sont précisément des mécanismes pour rétablir cette distinction à l'extérieur du LLM, puisqu'on ne peut pas l'imposer à l'intérieur.

---

## 13.2 Famille A — Tool poisoning et description spoofing

### 13.2.1 La première démonstration — Invariant Labs, mars 2025
...
### 13.2.2 Quatre variants documentés
...
### 13.2.3 L'asymétrie qui change tout
...
## 13.3 Famille B — Prompt injection cross-document et exfiltration

### 13.3.1 Le post-mortem GitHub README-to-Tool — Anthropic, juin 2025
...
C'est la deuxième famille d'attaques : la **prompt injection cross-document**. Elle exploite la propriété fondamentale identifiée au §13.1.2 — le LLM ne distingue pas instruction et donnée. ==Toute source de texte que le modèle lit via tool call est, potentiellement, un nouveau prompt système.==

![Chaîne d'exfiltration cross-document — quatre étapes|1300](../../mcp-securite/images/20260520-03-exfil-chain.svg)

### 13.3.2 Variants 2026
...
### 13.3.3 La défense load-bearing — HITL sur les `write` tools
...
## 13.4 Famille C — Cross-server confusion, namespace collisions, shadowing
...
### 13.4.2 La défense — allowlist namespace
...
## 13.5 Famille D — Transport, OAuth, supply chain
...
### 13.5.1 OAuth 2.0 + PKCE — quatre pitfalls
...
### 13.5.2 Supply chain — typosquatting, hijack, dépendance transitive
...
### 13.5.3 Sigstore — le pivot infrastructure
...
## 13.6 La matrice défensive — dix patterns, quatre load-bearing

### 13.6.1 La matrice 10×10
...
### 13.6.2 Les quatre patterns load-bearing
...
### 13.6.3 Les six patterns secondaires
...
## 13.7 Roadmap 12 mois — ce qui change d'ici mai 2027
...
### 13.7.3 Hiver 2026-2027 — Montée des registries signés
...
### 13.7.4 Printemps 2027 — Convergence MCP / A2A
