...
**Définir un sub-agent custom — exemple concret.** Un sub-agent vit dans un fichier markdown isolé, sous `.claude/agents/<nom>.md` (scope projet) ou `~/.claude/agents/<nom>.md` (scope user, disponible partout)[^3]. Le format est simple : un frontmatter YAML qui déclare l'identité et les contraintes, puis un corps markdown qui est le system prompt du sub-agent. Quatre champs frontmatter importants : `name` (l'identifiant invocable), `description` (utilisée par le matching automatique du main-thread pour décider quand déléguer), `tools` (la liste blanche des tools accordés — restreindre limite les dégâts potentiels et focalise le sub-agent), `model` (`haiku` pour les tâches simples et fréquentes, `sonnet` pour le complexe nuancé, `opus` pour le difficile). La commande `/agents` ouvre un assistant qui aide à drafter ces fichiers plutôt que de les écrire à la main — utile pour le frontmatter, qui plante silencieusement si la syntaxe YAML est cassée.

Un exemple utile pour un data analyst : un sub-agent qui cartographie en trente secondes un nouveau dataset, là où l'analyste mettrait dix minutes à requêter `INFORMATION_SCHEMA` ou à lire les `parquet metadata` à la main.

```yaml
---
name: schema-explorer
description: Explore le schéma d'une base relationnelle ou d'un dataset Parquet et produit un récap structuré (tables, colonnes, cardinalités, FK probables, valeurs anormales).
tools:
  - Read
  - Glob
  - Bash
...
Tu es un sub-agent spécialisé dans la cartographie rapide d'un schéma de données. À chaque invocation :

1. Identifie le format de la source (CSV, Parquet, base SQL via psql/sqlite, dossier dbt models/).
2. Pour chaque table/fichier : liste les colonnes, leur type inféré, les cardinalités, le taux de null.
3. Repère les colonnes-clés probables (id, fk_*, *_id) et les anomalies (cardinalité 1, taux de null > 80%, types incohérents).
4. Renvoie un récap structuré en sections : « Tables », « Colonnes-clés », « Anomalies », « Suggestions de profil approfondi ».

Tu n'écris JAMAIS dans la base. Tu n'exécutes JAMAIS de DDL. En cas de doute sur l'authorisation lecture, demande avant.

Réponse maximale : 800 mots, citer 3 priorités d'investigation à la fin.
```

==L'analyste face à un dataset inconnu invoque alors `Agent(subagent_type="schema-explorer")` et reçoit en trente secondes une carte du terrain== — colonnes, cardinalités, anomalies, priorités d'investigation. Sans avoir à se rappeler la requête `INFORMATION_SCHEMA.COLUMNS` ni à googler la commande `parquet-tools schema`. Le main-thread, lui, garde son contexte intact : le sub-agent a brûlé ses tokens dans son propre fil et n'a renvoyé qu'un résumé structuré de 600-800 mots.

**Pourquoi c'est différent d'une skill.** La skill (§2.3) est une procédure invocable dans le contexte courant : Claude charge le `.md` de la skill, lit les instructions, applique le protocole — tout cela consomme le budget contextuel du main-thread. Le sub-agent **isole** le travail dans un contexte propre. Trois conséquences pratiques. Premièrement, économie de tokens : le main-thread ne voit que le résumé final, pas les 20 000 tokens de `INFORMATION_SCHEMA` brassés en route. Deuxièmement, parallélisme possible : trois sub-agents `schema-explorer` peuvent tourner en même temps sur trois sources de données différentes, là où une skill exécutée trois fois saturerait séquentiellement le main-thread. Troisièmement, restriction de tools indépendante : le sub-agent ci-dessus n'a que `Read`, `Glob`, `Bash` — pas de `Write`, pas de `Edit`. Même si le system prompt dit *« tu n'écris jamais »*, le moteur d'autorisation refusera de toute façon. Ceinture et bretelles.

La règle pratique : ==si la procédure est légère et lue rarement, en faire une skill ; si elle est dense (cartographier un schéma, fouiller un repo, lancer un audit sécurité) ou parallélisable, en faire un sub-agent[^3].== Une organisation mûre finit par avoir les deux — une bibliothèque de skills pour les protocoles éditoriaux courts, et un catalogue de sub-agents pour les explorations denses.
...
**Les six modes en détail.** La granularité est une vraie force de Claude Code, mais aussi une charge cognitive — il faut savoir quel mode utiliser, et savoir basculer rapidement. Voici à quoi sert chacun, dans quel contexte l'employer[^5].

- **`default`** — mode standard. Les lectures (`Read`, `Glob`, `Grep`) sont auto-approuvées ; tout le reste (édits, `Bash`, écriture) prompte. *Quand l'utiliser* : la session de travail normale, où on veut garder la main sur les actions modifiantes. C'est le mode qu'on quitte rarement quand on découvre Claude Code.
- **`acceptEdits`** — mode *« j'ai confiance, déroule »*. Les édits filesystem (`Edit`, `Write`, `MultiEdit`) et les commandes shell de manipulation de fichiers (`mkdir`, `touch`, `mv`, `cp`, `rm` non destructif, `sed`) passent en auto. *Quand l'utiliser* : une fois qu'un plan a été validé et qu'on lance Claude pour exécuter mécaniquement vingt edits triviaux — typiquement les phases d'application après un brainstorming, ou les refactos guidés où on a déjà revu la stratégie.
- **`plan`** — mode revue. La recherche read-only est autorisée, mais l'agent **ne peut rien modifier** ; il rédige un plan structuré (objectif, étapes, risques) qu'il propose à l'utilisateur. *Quand l'utiliser* : démarrer un chantier complexe où on veut voir le plan avant d'engager du code, ou auditer un repo sans risque. C'est aussi le mode par défaut du sub-agent `Plan`.
- **`auto`** — mode *« modèle gardien »*. Un classifier IA séparé review chaque action proposée par l'agent et décide auto/prompt. Disponible avec Sonnet 4.6, Opus 4.6 ou Opus 4.7 — pas avec Haiku ni les modèles plus anciens. *Quand l'utiliser* : usage avancé où on veut une décision contextuelle (l'agent voulait `rm -rf` sur un dossier sensible ? le classifier prompte ; il voulait `npm test` ? il passe). Le compromis : un appel modèle supplémentaire par décision, donc un peu plus de latence et de coût, mais beaucoup moins d'interruptions injustifiées qu'en `default`.
- **`dontAsk`** — mode CI. L'agent refuse toute action qui n'est pas explicitement pré-approuvée dans `permissions.allow`. Pas de prompt, pas de bascule manuelle : si ce n'est pas listé, c'est refusé. *Quand l'utiliser* : runner CI, hook git, exécution headless dans un pipeline — partout où il n'y a pas d'humain pour répondre à un prompt. Demande de maintenir une allowlist exhaustive en amont.
- **`bypassPermissions`** — mode total. Tous les checks sont skippés, y compris les protected paths. *Quand l'utiliser* : un conteneur isolé jetable (sandbox Vercel, devcontainer éphémère, VM dédiée) où l'on accepte le risque maximal en échange d'une vélocité maximale. ==**Ne jamais activer ce mode sur un poste de travail développeur** — l'agent peut alors réécrire `.git/`, `.claude/`, `.mcp.json` et la configuration de l'IDE.==

Le switch entre modes est instantané. **`Shift+Tab`** cycle entre `default → acceptEdits → plan` (les trois modes les plus courants) ; le flag CLI `claude --permission-mode <mode>` impose un mode au lancement ; et la clé `permissions.defaultMode` dans `settings.json` fixe le mode par défaut au niveau projet ou user[^5]. En pratique, un workflow type alterne : `plan` pour cadrer, `default` pour exécuter avec supervision, `acceptEdits` pour les passes mécaniques validées, retour `default` ensuite. Le `Shift+Tab` devient un réflexe.

**Protected paths : la frontière en dur.** Quel que soit le mode (sauf `bypassPermissions`), certains paths ne sont jamais auto-approuvés en écriture : `.git`, `.vscode`, `.idea`, `.claude` (sauf les sous-dossiers `commands/`, `agents/`, `skills/`, `worktrees/` qui sont les zones de travail légitimes de l'agent sur lui-même), les rcs shell (`.bashrc`, `.zshrc`, `.profile`), `.mcp.json`, `.claude.json`. Cette liste est la garantie structurelle qui protège la configuration de l'agent contre l'agent. Sans elle, une session en `acceptEdits` pourrait laisser Claude réécrire son propre `CLAUDE.md`, modifier ses hooks, ajouter des serveurs MCP arbitraires — c'est précisément le scénario à éviter. ==C'est la dernière ligne de défense, et la seule manière de la franchir est `bypassPermissions` — qui doit donc être réservé aux environnements isolés où la perte de la configuration n'est pas un problème.==
