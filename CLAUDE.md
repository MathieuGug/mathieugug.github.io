...
- **Full-bleed sur les pages narratives** (journal, livre, scrolly) : sur les layouts qui ont un wrap étroit (typiquement `.wrap { max-width: 760px }`), les schémas SVG **cassent** le wrap pour occuper toute la viewport — texte à 760px, schémas pleine largeur. Le `<figcaption>` reste re-centré à 760px sinon sa typographie devient illisible sur grand écran. Pattern CSS : `.entry figure { margin: 26px calc(50% - 50vw); padding: 0; position: relative; }` + `.entry figure svg { width: 100%; border-radius: 0; }` + `.entry figure figcaption { max-width: 760px; margin: 10px auto 0; padding: 0 5vw; }`. Préalable : `html, body { overflow-x: hidden }` (déjà dans le boilerplate mobile-friendliness ci-dessous) pour que les marges négatives ne déclenchent pas de scroll horizontal. Cas de référence : `proces-musk-altman/journal.html`.
- **Full-bleed aussi sur les apps deep-research desktop** (`header.site` + `main#report` + sidebars) : sous `@media (min-width: 1025px)`, la `.figure` casse aussi son `main#report` (max-width 760px) pour s'étaler sur toute la viewport. Le texte du rapport garde ses 760px, les schémas occupent l'écran entier. Compromis assumé : sur desktop, le schéma **recouvre les sidebars TOC et Sources** pendant qu'il est dans la viewport — c'est le prix de la lisibilité (un schéma d'architecture à 664px utiles sur un 1920px gaspille la moitié de l'écran). La grille étant asymétrique (TOC 240 / 1fr / sources 320), le centrage est imparfait à ~40px près — visuellement OK. Mobile (<1025px) : la grille s'effondre déjà en colonne unique, la figure remplit naturellement, pas de règle spécifique. Pattern CSS et reasoning détaillés dans la skill `illustrated-deep-research` (`references/companion-app.md` § "Full-bleed schemas on desktop"). Cas de référence (à appliquer ou pas selon ressenti par app) : à ajouter dans les six études existantes si on veut harmoniser ; tout nouveau rapport généré par la skill embarque déjà la règle.
