...
- **Full-cell sur les apps deep-research desktop** (`header.site` + `main#report` + sidebars) : sous `@media (min-width: 1025px)`, la `.figure` casse `main#report` (max-width 760px) pour remplir la **largeur entière du main-cell** de la grille — entre TOC à gauche et Sources à droite. **Aucun chevauchement des sidebars** (contrainte stricte : avec `width: 100vw`, le background `--paper` de la figure peint au-dessus des items du TOC et les masque visuellement). Le texte garde ses 760px, les schémas gagnent ~120px de largeur utile sur 1440+. Pattern CSS : `.figure { width: min(calc(100vw - 560px), 880px); margin-left/right: calc(-1 * (max(0px, min(60px, (100vw - 1320px) / 2)) + 48px)); padding-left/right: clamp(16px, 3vw, 48px); border-radius: 0; }` sous `@media (min-width: 1025px)`. Math, dérivation, et garde-fous dans la skill `illustrated-deep-research` (`references/companion-app.md` § "Full-cell schemas on desktop"). Mobile (<1025px) : la grille s'effondre en colonne unique, la figure remplit naturellement. Appliqué aux sept apps existantes (`agents-computer-use`, `evaluation-agentique`, `harness-agentique`, `llm-jailbreaking`, `memoire-agentique`, `observabilite-agents-ia`, `proces-musk-altman/20260427-…-app.html`) ; tout nouveau rapport généré par la skill l'embarque par défaut.
