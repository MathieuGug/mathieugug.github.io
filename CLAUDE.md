...
- **Apps deep-research** : grille `.layout` pleine viewport (sidebars sur les bords) ; figure adaptative aux deux états sidebar via `margin-inline: calc(-1 * max(0px, (100vw - 1320px) / 2 + 48px))` (formule dérivée pour un `main#report` container-capped à 760 px — prérequis structurel).
