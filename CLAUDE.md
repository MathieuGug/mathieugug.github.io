...
- **Largeur des schémas adaptative selon l'état des sidebars** sur les apps deep-research desktop (`header.site` + `main#report` + sidebars) : sous `@media (min-width: 1025px)`, la `.figure` casse `main#report` (max-width 760px) pour prendre **toute la place horizontale visible**, en s'arrêtant pile aux sidebars actuelles. **Deux états** :
  - **Sources ouvertes (par défaut)** : figure remplit le main-cell de la grille (entre TOC et Sources). ~880px sur layout 1440+.
  - **Sources repliées (`.layout.sources-collapsed`)** : figure s'étend du bord droit du TOC jusqu'au bord droit de la viewport. ~1200px sur 1440 / ~1440px sur 1920.

  **Aucun chevauchement des sidebars visibles** (contrainte stricte : avec `width: 100vw`, le background `--paper` de la figure peint au-dessus des items du TOC et les masque visuellement). Transition smoothe 280ms quand l'utilisateur toggle Sources, alignée avec la transition `grid-template-columns` du layout. Pattern CSS et formules complètes dans la skill `illustrated-deep-research` (`references/companion-app.md` § 5). Mobile (<1025px) : la grille s'effondre en colonne unique, la figure remplit naturellement, pas d'override. Appliqué aux sept apps existantes (`agents-computer-use`, `evaluation-agentique`, `harness-agentique`, `llm-jailbreaking`, `memoire-agentique`, `observabilite-agents-ia`, `proces-musk-altman/20260427-…-app.html`) ; tout nouveau rapport généré par la skill l'embarque par défaut.
