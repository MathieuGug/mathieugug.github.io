...
- **Relecture anti-tournures IA obligatoire** sur toute prose publiée (rapports, chapitres du livre, journaux, hubs, prose des apps). La liste mesurée sur le corpus, avec verdicts et quotas : `.claude/skills/illustrated-deep-research/references/red-flags-ia.md`. Les cinq durs : tirets cadratins ≤ 5/1000 mots ; pivot « — c'est / — pas » ≤ 2 par document ; négation appositive « X, pas Y » ≤ 2 ; antithèse « X n'est pas Y, c'est Z » bannie (trancher : garder la moitié sourcée) ; « c'est précisément » jamais. La règle qui gouverne tout : une phrase qui apporte du rythme sans apporter de fait se supprime — elle ne se reformule pas. Mesure d'ensemble avant/après nettoyage : `python3 tools/analyse_formules_ia.py --only md`.
...
- **Apps deep-research** : grille `.layout` pleine viewport (sidebars sur les bords) ; figure adaptative aux deux états sidebar, **capée à 80 % de la largeur disponible** via `margin-inline: calc(-1 * max(48px, 40vw - 556px))` (`40vw - 428px` sources repliées). Formule dérivée pour un `main#report` container-capped à 760 px — prérequis structurant. Le plancher à 48 px empêche la figure de passer sous la largeur de la colonne de texte sur les portables ~1440.
...
Pattern complet (quand l'utiliser, calibrage 6 axes × 3 inputs, schéma JSON, 5 profils + adjustments, anatomie HTML rendue, gotchas) : `.claude/skills/illustrated-deep-research/references/qualif.md`. Spec & plan d'origine : `docs/superpowers/specs/2026-05-23-business-qualification-widget-design.md`, `docs/superpowers/plans/2026-05-23-business-qualification-widget.md`.

**Pilotes déployés** : `analytics-agentique-gcp/`, `surfaces-agentiques/`.
...
Le script accepte les anchors `<h[23] id="...">` (convention `analytics-agentique-gcp`) **et** `<section id="...">` (convention `surfaces-agentiques`).

**Tests CI** : `tests/qualif-contract.test.mjs`, `tests/qualif-engine.test.mjs`, `tests/qualif-integration.test.mjs`, `tests/qualif-surfaces-integration.test.mjs` (workflow `.github/workflows/test.yml`).
