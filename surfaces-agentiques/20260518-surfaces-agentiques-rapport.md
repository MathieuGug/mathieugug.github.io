---
title: Surfaces agentiques
subtitle: Du chatbot au copilot on-behalf-of — comment l'utilisateur final accède à un agent
date: 2026-05-18
...
> *Le chatbot est la prise minimale, pas la bonne forme. La question d'accès à un agent va se reformuler en problème d'expérience utilisateur — et le bon design commence par décider du régime avant de coder l'agent.* — Mathieu Guglielmino, 18 mai 2026

## Synthèse exécutive

- **La surface mange le modèle.** Ben Thompson l'écrit en mars 2026 à propos du lancement de Microsoft Copilot Cowork à 99 $/siège : *« livrer un produit agentique convaincant exige d'abandonner l'objectif d'être model-agnostic. Un agent, c'est un modèle ET un harness, deux pièces intégrées »*[^1]. Le harness — donc, in fine, la **surface d'accès** que l'utilisateur final voit — est devenu le centre de gravité concurrentiel. Sierra (Bret Taylor) vaut 15,8 milliards de dollars en mai 2026[^2] sans avoir entraîné un seul modèle ; sa valeur tient à la surface customer-service qu'elle construit autour des LLM existants.
- ==**Quatre régimes d'accès, pas un.**== Le chat de ChatGPT a installé la croyance qu'on parle à un agent comme à un humain dans une fenêtre vide. Trois autres régimes ont émergé depuis : le **copilote inline** (l'IA dans l'outil — Cursor Tab, GitHub Copilot, Notion `/AI`) ; le **canvas génératif** (l'IA émet une interface — Vercel v0, Claude Artifacts, OpenAI Canvas) ; l'**agent on-behalf-of** (l'IA exécute en VM ou dans le navigateur — Operator, Computer Use, Devin, Manus). Choisir le bon régime *avant* de coder l'agent change tout : ce sont des architectures, des contrats utilisateur et des risques différents.
- **Le procès du chatbot a déjà eu lieu.** Amelia Wattenberger (*« chat is the wrong UI »*, 2023[^3]), Maggie Appleton (*Language Model Sketchbook*, 2023[^4]), Linus Lee (*Imagining better interfaces*[^5]), Geoffrey Litt (*malleable software*[^6]) et plus récemment Karri Saarinen (Linear, *Design for the AI Age*, avril 2025[^7]) et Allen Pike (*Post-Chat UI*, 30 avril 2025[^8]) ont produit en deux ans le procès du chat comme surface par défaut. Diagnostic convergent : pas d'affordance, pas de flow, manipulation indirecte, articulation forcée. Le Nielsen Norman Group le rejoint en 2024 avec *Overcoming the Articulation Barrier*[^9]. Le chat reste utile pour la *découverte* ; il est rarement la bonne surface pour l'*usage continu*.
- **AG-UI est le protocole qui standardise la prise.** Né en mai 2025 chez CopilotKit, AG-UI (*Agent-User Interaction*) formalise le tuyau d'événements entre un backend agentic stateful et une UI[^10]. Pas un framework, un transport — 17 événements core (lifecycle, message, tool call, state delta), bi-directionnel, agnostique de transport. Adopté par LangGraph, CrewAI, Mastra, Microsoft Agent Framework, Google ADK, AWS Strands, Pydantic AI, Agno, LlamaIndex, AG2, et — en tier community — par le Claude Agent SDK[^11]. ==C'est le pendant de [MCP](../mcp-plateforme/) côté frontend== : la stack agentique se cale sur trois piliers — MCP (agent ↔ tools), A2A (agent ↔ agent), AG-UI (agent ↔ utilisateur).
- **Le canvas est devenu un IDE.** Vercel v0 n'est plus un générateur de composants : c'est un éditeur Next.js full-stack hébergé avec Git, sandbox, DB Supabase/Snowflake — 3 millions d'utilisateurs déclarés au podcast Sequoia *Training Data*[^12], 10 % des signups Vercel viennent désormais de ChatGPT qui recommande v0. Claude Artifacts a été consolidé en avril–mai 2026 dans une **VM unifiée** qui exécute code, fichiers, Skills, Computer Use et MCP dans le même sandbox[^13]. Lovable a fait 100 M$ d'ARR en 8 mois sur la même promesse en B2C[^14]. La frontière code/no-code s'est dissoute en 12 mois.
- **L'agent on-behalf-of n'est viable commercialement qu'à 0,05–0,15 $ par tâche.** Le coût par tâche d'agent navigateur est passé de 0,50–1,50 $ (2024) à 0,05–0,15 $ (2026), et la précision GUI a franchi 90 % avec Claude Opus 4.7 et GPT-5[^15]. ==C'est cette double bascule économique qui débloque la vague de production==, pas un breakthrough modèle isolé. Anthropic Claude Computer Use atteint 44 % au benchmark OSWorld (×3 en un an) ; Operator OpenAI fait 38,1 % ; Convergence AI Proxy passe 88 % sur WebVoyager.
...
Une autre formulation, simple et opératoire, vient de la communauté open-source. *« A chatbot answers, a copilot suggests, an agent acts, and a digital employee persists »*, écrit Taskade en analysant la taxonomie de l'industrie début 2026[^18]. Cette gradation — *répond / suggère / agit / persiste* — capture la trajectoire d'autonomie croissante des produits AI 2022–2026. À chaque cran, la surface change. Et à chaque cran, l'utilisateur abandonne une part du contrôle de bas niveau qu'il maîtrisait — la frappe (copilot), la composition (canvas), l'exécution (agent autonome).

Le mot *surface* mérite d'être ancré. J'appelle ici **surface d'accès** à un agent l'ensemble des points d'interaction visibles côté utilisateur : la fenêtre, les contrôles, les feedbacks, le rendu des résultats intermédiaires, les options de correction et d'interruption. C'est tout ce qui n'est pas le modèle ni le harness backend (couvert dans les dossiers [*Agent SDK*](../agent-sdk/) et [*Harness agentiques*](../harness-agentique/)). C'est le contrat d'expérience que le produit pose au moment où l'utilisateur s'en sert.

==La surface est aussi ce qui matérialise les choix de design d'autonomie==. Le Knight First Amendment Institute de Columbia a publié en juillet 2025 un papier important — *Levels of Autonomy for AI Agents* — qui formalise cinq rôles utilisateur (operator, collaborator, consultant, approver, observer) selon le degré d'autonomie accordé à l'agent[^19]. La thèse-clé : *l'autonomie n'est pas un sous-produit de la capacité du modèle, c'est une décision de design — et elle doit être exposée explicitement à l'utilisateur*. On y revient au §8.

![La pile des quatre régimes d'accès à un agent — chat, inline, canvas, on-behalf-of|1300](images/20260518-01-pile-regimes.svg)

*Schéma 1 — Les quatre régimes d'accès à un agent en 2026. À chaque régime, l'unité d'interaction change, et avec elle ce que l'utilisateur abandonne et ce qu'il garde.*
...
**Allen Pike, *Post-Chat UI*** (30 avril 2025[^8]). Pike fait la synthèse pratique. Le chat reste utile comme *debug interface* et comme *fallback mode* — quand on ne sait pas ce qu'on cherche, le langage naturel est un point d'entrée acceptable. Mais pour 90 % des usages routiniers, il propose dix patterns alternatifs : *co-authorship* (Canvas, Cursor), *generative right-click* (Dia), *intuitive search* (Superhuman, Figma), *type instead of pick*, *inline feedback* (writing daemons), *clean up* (Figma Rename Layers), *summary/synthesis* (Apple Intelligence), *voice + pointing*, *do-the-obvious-thing* (tab complete), *completely generated UI* (bolt.new, avec un avertissement sur la learnability).

Que reste-t-il de positif au chat après ce procès ?

**Trois choses, et ce sont les bonnes raisons de garder le chat dans la stack** :
...
==Le verdict synthétique : le chat est la **prise minimale**, pas la **bonne forme**.== C'est le port USB universel — utile pour brancher n'importe quoi, jamais la solution la plus élégante pour un usage spécifique. La suite du rapport explore les surfaces qu'on construit *après* cette reconnaissance.

![Le procès du chatbot — affordance, flow, articulation|1300](images/20260518-02-proces-chatbot.svg)

*Schéma 2 — Trois griefs convergents et trois usages qui restent légitimes. Le chat survit comme port universel ; il cesse d'être la surface de référence.*
...
La force du sub-mode : *négociation et raisonnement explicite*. L'utilisateur peut demander pourquoi, demander une alternative, contraindre la sortie. Sa limite : on retombe partiellement dans les griefs du §2 — il faut articuler, attendre, évaluer.
...
- **Google Workspace + Gemini** — embeddé par défaut depuis mars 2026 dans Gmail/Docs/Sheets/Slides/Drive/Meet/Chat[^24]. *Help me create* synthétise depuis fichiers et emails en un draft formaté. *Workspace Intelligence* introduit des relations sémantiques cross-app.
...
- **Linear AI** — pas un copilote au sens classique : Linear intègre nativement **Codex / Claude Code / Cursor / Copilot / Devin**, et devient l'orchestrateur des agents de coding qui modifient le ticket. La conférence TBPN de mars 2026 fait dire à son CEO *« issue tracking is dead »* — Linear devient *« context infrastructure »* pour agents[^7].
...
![Trois sub-modes inline — ghost text, sidebar chat, agent mode|1300](images/20260518-03-trois-submodes-inline.svg)

*Schéma 3 — Les trois sub-modes du copilote inline. Ils ne se canniba­lisent pas — ils s'empilent, et les bons produits 2026 basculent entre eux sans rupture cognitive.*
...
Quatre régimes coexistent sous le terme *generative UI*, et il est important de les distinguer — ce sont des architectures et des risques différents. CopilotKit propose la taxonomie la plus claire à date[^27] :
...
3. **Open-ended.** L'agent émet directement du HTML/JS exécuté en sandbox (typiquement iframe ou shadow DOM). C'est ce que fait Claude Artifacts, ChatGPT Canvas, Vercel v0. Liberté maximale, risque maximal.
4. **Dynamic data-driven.** L'UI elle-même est pré-codée, mais ses paramètres et son contenu varient selon une requête NL → SQL → visualisation. C'est le régime de la BI conversationnelle (Tableau Pulse, ThoughtSpot Spotter, Sigma AI, Sisense GenBI).
...
- **Claude Artifacts + Skills + VM unifiée.** Trajectoire à connaître pour positionner Anthropic dans le paysage[^13] :
...
- **Microsoft Copilot Pages + Loop.** Microsoft maintient trois surfaces distinctes : *Pages* (canvas généré par Copilot, équivalent direct de Canvas/Artifacts), *Loop* (co-authoring temps réel + composants portables synchronisés où qu'ils soient collés), *Copilot Notebooks* (contextuel profond, knowledge workbench)[^31]. Le leak *Copilot Canvas* (Windows Latest, mars 2026) ajoute whiteboard + image gen + AI streaming live.
...
- **Gamma** — présentations web-first narratives. Free tier 400 crédits, Plus 10 $/mo, Pro 20 $/mo. Best-in-class pour les decks consommés en ligne (pas en `.pptx`). Tome a sunset son slide product le 30 avril 2025 et pivoté vers sales automation — à ne plus citer comme alternative présentation.
...
==Le centre de gravité 2026 a basculé vers MCP==. Construire pour MCP Apps maintenant, c'est construire une fois pour Claude + ChatGPT + VS Code Copilot + Goose. Construire pour Apps SDK seul, c'est accepter le lock-in ChatGPT. Construire pour A2UI seul, c'est parier sur la sécurité avant la reach.

![Quatre régimes de generative UI — controlled, declarative, open-ended, dynamic data-driven|1300](images/20260518-04-regimes-genui.svg)

*Schéma 4 — Quatre régimes de generative UI, du plus contraint au plus libre. Le bon design choisit *quel* régime pour quelle tâche — c'est une décision d'architecture, pas un détail d'implémentation.*

### Pourquoi le canvas gagne sur certaines tâches

==Le canvas est la bonne surface quand la tâche produit un *objet*, pas une *décision*==. Une slide, un dashboard, un composant, une mini-app, un rapport — autant d'objets que l'utilisateur veut voir, manipuler, éditer, partager. Le chat seul échoue ici parce qu'il oblige à itérer sur du texte alors que l'objet final doit être visuel/interactif.

Sa limite : le canvas n'est pas la bonne surface quand la tâche a un *résultat dans le monde* — envoyer un mail, exécuter un transfert bancaire, fixer un rendez-vous, faire fonctionner un workflow en production. Pour ça, on a besoin du régime 4 (on-behalf-of). Avant, on passe par le régime 3 — l'expérience narrative orientée tâche.
...
J'ai consacré un dossier entier à la catégorie en mai 2026 — [*Expériences narratives*](../narrative-experiences/) — qui retrace la généalogie depuis Segel et Heer (spectre auteur ↔ lecteur, sept genres, 2010[^34]) jusqu'aux scrollytellers contemporains. Ce que ce régime apporte au monde agentique : *le mélange d'une structure auctoriale (l'agent sait où il va) et de poignées calibrées (l'utilisateur peut explorer à des endroits précis du parcours)*.

### Trois symptômes que ce régime émerge

**Symptôme 1 : Live Artifacts.** Anthropic a sorti les Live Artifacts en avril 2026[^13]. Ce ne sont plus des chunks HTML statiques, ce sont des **applications persistantes** qui se rafraîchissent à chaque ouverture via MCP. Concrètement, un utilisateur peut demander à Claude *« construis-moi un tracker des leads chauds de cette semaine »* — l'agent compose une mini-app qui requête Gmail/Calendar/Slack via MCP, affiche les leads dans un layout adapté, propose des actions (envoyer un follow-up, scheduler un appel). C'est de la generative UI au sens technique, mais c'est aussi une *expérience narrative* au sens éditorial : l'agent a fait un choix de scénario (cette semaine, leads chauds, follow-up = action prioritaire), il a hiérarchisé l'information, il a calibré les poignées d'action.

**Symptôme 2 : Tableau Pulse.** Salesforce a poussé Pulse depuis 2024 sur un format proche : *résumés AI proactifs* sur des métriques pré-définies, envoyés en email ou Slack, avec une narrative générée et trois actions suggérées[^35]. Ce n'est pas un dashboard explorable — c'est un *brief* éditorial calibré pour une tâche (typiquement : décider quoi prioriser cette semaine). La différence avec un dashboard classique : la donnée ne se laisse pas explorer librement, l'utilisateur reçoit un parcours.

**Symptôme 3 : Sierra.** Sierra (Bret Taylor) ne fait pas de dashboards et n'écrit pas d'apps. Sierra fait des **agents customer-service**, et la surface utilisateur — du côté du client final — n'est pas un chat avec une IA, c'est un *appel téléphonique ou une conversation Web*, scénarisé par l'agent, qui sait où la conversation doit aller. Valorisée 15,8 Md $ en mai 2026[^2], plus de 150 M$ ARR à février, 40 % du Fortune 50 en clientèle. ==C'est la radicalisation du régime 3 : la surface n'est même plus une UI au sens classique, c'est un parcours conversationnel scénarisé orienté vers une issue de tâche==.

### Trois principes pour designer une expérience narrative agentique

Tirés des sept genres de Segel-Heer adaptés à l'IA :

1. **L'auteur sait où il va.** L'agent ne demande pas *« comment veux-tu structurer ça ? »*. Il propose une structure et négocie ensuite les écarts. C'est ce que Tableau Pulse fait avec ses résumés, c'est ce que les Live Artifacts font en pré-calculant le layout, c'est ce que Sierra fait en scénarisant l'appel.
2. **Des poignées calibrées, pas une liberté totale.** L'utilisateur peut zoomer sur une cellule, demander un détail, intervenir à un point précis du parcours. Mais il ne peut pas tout reconfigurer — ça casserait la cohérence narrative et obligerait à re-générer toute la UI.
3. **Une métrique de qualité explicite.** Une expérience narrative est *bonne* quand l'utilisateur a compris quelque chose qu'il ne savait pas avant, et qu'il peut agir dessus. Ce n'est ni *la couverture* (régime dashboard) ni *la mémorisation* (régime infographique) — c'est *la conversion en décision*. C'est ce qui distingue un brief utile d'un dashboard noyé.
...
Pattern critique systématisé par Salesforce[^46]. L'agent identifie les scenarios sensibles ou complexes — sentiment utilisateur négatif, tâche requérant des permissions humaines — et fait un *warm hand-off* à un humain. Le handoff porte le contexte de la conversation. C'est *warm* parce qu'il ne force pas l'utilisateur à tout recommencer.

![La surface de l'agent on-behalf-of — quatre questions UX et leurs patterns dominants|1300](images/20260518-05-surface-onbehalfof.svg)

*Schéma 5 — Les quatre questions UX critiques pour un agent on-behalf-of. Les patterns dominants convergent à travers l'industrie. Un produit qui ne propose pas ces quatre primitives part avec un déficit de confiance.*
...
Le pitch officiel : *« an open, lightweight, event-based protocol that standardizes how AI agents connect to user-facing applications »* — *« the general-purpose, bi-directional connection between a user-facing application and any agentic backend »*. Le pitch journalistique *« MCP for the UI layer »* est juste sur le fond, mais la formulation officielle est plus précise : ==AG-UI est **le pendant frontend** de MCP côté tools — la stack agentique se cale sur trois piliers, MCP / A2A / AG-UI==.

![Trois piliers — MCP côté tools, A2A entre agents, AG-UI côté utilisateur|1300](images/20260518-06-trois-piliers.svg)

*Schéma 6 — Les trois standards qui structurent la stack agentique 2026. MCP connecte l'agent aux outils et données ; A2A coordonne les agents entre eux ; AG-UI relie l'agent à l'application utilisateur.*
...
**17 événements core** + extensions Reasoning/Activity/Meta en draft. Catalogue :
...
La distinction essentielle : *shared state = « qu'est-ce que l'agent sait » ; generative UI = « qu'est-ce que l'agent montre »*. AG-UI transporte les deux, mais ce sont des patterns architecturaux orthogonaux.

![Flux d'événements AG-UI — séquence type d'un run avec text, tool call et state delta|1300](images/20260518-07-flux-agui.svg)

*Schéma 7 — Une session AG-UI lue comme une partition. À gauche, l'agent émet la séquence d'événements ; à droite, le frontend rend chaque événement à sa façon — text streamé, tool call dépliable, state delta appliqué à l'UI.*
...
**SDKs langages** : Python (34 % de la codebase), TypeScript (30 %), Kotlin, Java, Go, C++, Ruby, Dart, Rust. **Pas de SDK .NET stable** au 18 mai 2026.
...
**Question de maturité à flagger** : AG-UI est un standard d'un an, son écosystème est en pleine consolidation. L'intégration Vercel AI SDK est annoncée mais non publiée sur npm au 18 mai 2026[^51]. L'OpenAI Agent SDK et AWS Bedrock Agents sont en *in progress*. ==Adopter AG-UI en mai 2026 = parier sur un standard émergent largement adopté mais pas encore stabilisé sur toutes ses extensions.==
...
Le papier le plus important de 2025 sur ces questions est *Levels of Autonomy for AI Agents* du Knight First Amendment Institute de Columbia (juillet 2025)[^19]. Il définit cinq rôles utilisateur, du moins au plus autonome :
...
![Cinq niveaux d'autonomie — operator, collaborator, consultant, approver, observer|1300](images/20260518-08-niveaux-autonomie.svg)

*Schéma 8 — L'échelle d'autonomie du Knight First Amendment Institute. À chaque niveau correspond un pattern UX distinct. L'autonomie est exposée explicitement à l'utilisateur, jamais cachée.*
...
2. **Graduated trust spectrum.** Les données empiriques d'Anthropic montrent que l'auto-approve passe de ~20 % à 50 sessions à >40 % à 750 sessions. **L'autonomie est co-construite par l'usage**, pas configurée une fois pour toutes.
3. **Defense in depth.** Plusieurs couches imparfaites. Cf. le schéma *gruyère suisse* du dossier [*La fabrique d'un agent*](../fabrique-agent/) : alignement modèle + harness controls + sandbox env.
...
Le **modèle de responsabilité partagée** d'Anthropic, en quatre couches : *Model* (owned par le provider), *Harness* (instructions et policies, partagé), *Tools* (MCP servers, APIs, owned par l'organisation déployante), *Environment* (sandboxes, data, owned par l'orga).
...
**Couche 1 — Agent backend.** Le moteur. C'est tout ce qui est traité dans le dossier [*Agent SDK*](../agent-sdk/) et dans [*Harness agentiques*](../harness-agentique/) : le harness Claude Code / LangGraph / Mastra / Pydantic AI, le model, les tools, les skills, les hooks, le filesystem, le context management. Pour ce rapport, ça reste une boîte noire — la surface est ce qui le consomme, pas ce qu'il y a dedans.
...
**Couche 5 — Guardrails et observability.** Les hooks d'interception (cf. [Agent SDK](../agent-sdk/) §6-8), le permission system, le logging exhaustif, l'audit log côté UI, les pop-up de confirmation contextuelle. Pour la production, ajouter le monitoring de coûts par tâche et la *replay capability* — pouvoir rejouer une session pour débugger.

![Architecture canonique d'une surface agentique — cinq couches|1300](images/20260518-09-architecture-canonique.svg)

*Schéma 9 — L'architecture canonique d'une surface agentique en cinq couches. La séparation AG-UI / frontend runtime est ce qui permet la portabilité entre framework agentic. La couche generative UI est ce qui définit le régime d'usage (canvas, narratif, dashboard).*
...
==Aucune surface sérieuse ne fait l'économie des cinq couches.== Beaucoup de produits 2024-2025 ont fait l'erreur de skipper la couche 5 (les démos flashy ne montrent pas les guardrails) et l'ont payé en perte de confiance utilisateur.
...
| Dashboard sur métriques pré-définies | **Narratif orienté tâche** | Tableau Pulse, Live Artifacts | Semantic layer gouvernée |
...
Pour aller plus loin sur le moteur derrière la surface : [*Agent SDK*](../agent-sdk/) (le harness Claude Code et ses trois voies de build), [*Harness agentiques*](../harness-agentique/) (les sept couches industrielles), [*Coding agents*](../coding-agents/) (panorama produits Claude Code / Cursor / Aider / Devin), [*La fabrique d'un agent*](../fabrique-agent/) (organisation d'équipe). Pour la lignée des expériences narratives : [*Expériences narratives*](../narrative-experiences/) (Segel-Heer, Bertin, Tufte, Cairo, Lupi, Fragapane). Pour l'écosystème MCP : [*MCP comme plateforme*](../mcp-plateforme/).
...
[^6]: **Geoffrey Litt et al.** (Horowitz, van Hardenberg, Matthews), *Malleable software: Restoring user agency in a world of locked-down apps*, Ink & Switch, 2025. [inkandswitch.com/essay/malleable-software](https://www.inkandswitch.com/essay/malleable-software/). Voir aussi l'essai initial *Malleable software in the age of LLMs*, mars 2023, [geoffreylitt.com/2023/03/25/llm-end-user-programming](https://www.geoffreylitt.com/2023/03/25/llm-end-user-programming.html). Consulté 2026-05-18.
...
[^13]: **Anthropic**, *What are artifacts and how do I use them?*, Claude Help Center. [support.claude.com/en/articles/9487310-what-are-artifacts-and-how-do-i-use-them](https://support.claude.com/en/articles/9487310-what-are-artifacts-and-how-do-i-use-them). Annonce Skills, 16 octobre 2025. Live Artifacts (Cowork), 21 avril 2026. Migration VM unifiée, notification admin 13 mai 2026, [toolnav.io/news/2026-05-13-anthropic-claude-vm-artifacts-skills-migration](https://toolnav.io/news/2026-05-13-anthropic-claude-vm-artifacts-skills-migration). Consulté 2026-05-18.

[^14]: **Lovable**, plateforme. [lovable.dev](https://lovable.dev). Voir aussi reporting Series B 330 M$, valo 6,6 Md $, fin 2025 (TechCrunch, The Information). Consulté 2026-05-18.

[^15]: **VentureBeat**, *The rise of browser-use agents: why Convergence's Proxy is beating OpenAI's Operator*. [venturebeat.com/ai/the-rise-of-browser-use-agents-why-convergences-proxy-is-beating-openais-operator](https://venturebeat.com/ai/the-rise-of-browser-use-agents-why-convergences-proxy-is-beating-openais-operator). Benchmarks OSWorld et WebVoyager publics. Évolution coût/tâche : *AgentMarketCap, Voice AI Agents kill the call-center*, 5 avril 2026. [agentmarketcap.ai/blog/2026/04/05/voice-ai-agents-kill-the-call-center](https://agentmarketcap.ai/blog/2026/04/05/voice-ai-agents-kill-the-call-center). Consulté 2026-05-18.

[^16]: **Andrej Karpathy**, *Software Is Changing (Again)*, Sequoia AI Ascent 2026, 29 avril 2026. Résumé Karpathy bearblog. [karpathy.bearblog.dev/sequoia-ascent-2026](https://karpathy.bearblog.dev/sequoia-ascent-2026/). Et conf YC AI Startup School, juin 2025, *Software Is Changing Again*, [latent.space/p/s3](https://www.latent.space/p/s3). Consulté 2026-05-18.
...
[^22]: **InfoQ**, *Cursor 3 ships agent-first interface*, avril 2026. [infoq.com/news/2026/04/cursor-3-agent-first-interface](https://www.infoq.com/news/2026/04/cursor-3-agent-first-interface/). Voir aussi reporting valo, *Antigravity Lab*. [antigravitylab.net/en/articles/ai-tools/ai-ide-market-windsurf-acquisition-2026](https://antigravitylab.net/en/articles/ai-tools/ai-ide-market-windsurf-acquisition-2026). Consulté 2026-05-18.
...
[^29]: **Vercel**, *Introducing AI SDK 3.0 with Generative UI support*, 1 mars 2024. [vercel.com/blog/ai-sdk-3-generative-ui](https://vercel.com/blog/ai-sdk-3-generative-ui). Et *Open in v0* / shadcn registry, [ui.shadcn.com/docs/registry/open-in-v0](https://ui.shadcn.com/docs/registry/open-in-v0). Consulté 2026-05-18.
...
[^31]: **Microsoft Support**, *Compare Microsoft Loop, Copilot Pages, and Copilot Notebooks*. [support.microsoft.com/en-us/topic/compare-microsoft-loop-copilot-pages-and-copilot-notebooks-d6bf4dc0-7b09-4f54-9a92-ff898b8397f7](https://support.microsoft.com/en-us/topic/compare-microsoft-loop-copilot-pages-and-copilot-notebooks-d6bf4dc0-7b09-4f54-9a92-ff898b8397f7). Consulté 2026-05-18.

[^32]: **Replit**, *Agent 4 release*, mars 2026. [replit.com](https://replit.com). Voir aussi *Computerworld* sur Copilot Pages, [computerworld.com/article/3523720/microsoft-revamps-m365-copilot-chatbot-with-pages-shared-canvas.html](https://www.computerworld.com/article/3523720/microsoft-revamps-m365-copilot-chatbot-with-pages-shared-canvas.html). Consulté 2026-05-18.
...
[^34]: **Edward Segel & Jeffrey Heer**, *Narrative Visualization: Telling Stories with Data*, IEEE TVCG, 2010. Voir aussi le dossier [*Expériences narratives*](../narrative-experiences/) sur ce site pour la généalogie complète. Consulté 2026-05-18.
...
[^41]: **Cognition**, *Introducing Devin*. [cognition.ai/blog/introducing-devin](https://cognition.ai/blog/introducing-devin). Et *Devin for Terminal*, [cognition.ai/blog/devin-for-terminal](https://cognition.ai/blog/devin-for-terminal). Consulté 2026-05-18.
...
[^43]: **Anthropic**, *Our framework for developing safe and trustworthy agents*. [anthropic.com/news/our-framework-for-developing-safe-and-trustworthy-agents](https://www.anthropic.com/news/our-framework-for-developing-safe-and-trustworthy-agents). Et *Responsible Scaling Policy v3.0*, [anthropic.com/responsible-scaling-policy](https://www.anthropic.com/responsible-scaling-policy). Consulté 2026-05-18.
...
[^45]: **Wen Shelly et al.**, *WaitGPT: Monitoring and Steering Conversational LLM Agent in Data Analysis with On-the-Fly Code Visualization*, UIST 2024. [arxiv.org/html/2408.01703v1](https://arxiv.org/html/2408.01703v1). Project page [shellywhen.github.io/projects/WaitGPT](https://shellywhen.github.io/projects/WaitGPT). Consulté 2026-05-18.
...
[^49]: **CopilotKit Documentation**, *useAgent* + *useCoAgent* hooks. [docs.copilotkit.ai/learn/ag-ui-protocol](https://docs.copilotkit.ai/learn/ag-ui-protocol). Et [docs.copilotkit.ai/reference/hooks/useCoAgent](https://docs.copilotkit.ai/reference/hooks/useCoAgent). Consulté 2026-05-18.

[^50]: **Vercel**, *AI SDK RSC streamUI status*, ai-sdk.dev. [ai-sdk.dev/docs/reference/ai-sdk-rsc/stream-ui](https://ai-sdk.dev/docs/reference/ai-sdk-rsc/stream-ui). Note officielle de pause du développement RSC. Consulté 2026-05-18.
...
[^54]: **Andrej Karpathy**, *Software Is Changing (Again)*, YC AI Startup School, juin 2025. Transcript [donnamagi.com/articles/karpathy-yc-talk](https://www.donnamagi.com/articles/karpathy-yc-talk). Vidéo Latent Space [latent.space/p/s3](https://www.latent.space/p/s3). Consulté 2026-05-18.

## Notes

- Format co-écrit avec l'aide d'une IA.
- Ce dossier complète [*Agent SDK*](../agent-sdk/) (le moteur côté backend) avec la question symétrique : *par quelles surfaces les utilisateurs accèdent-ils à un agent ?* Les deux rapports sont à lire de pair.
- Voir aussi sur ce site : [*Harness agentiques*](../harness-agentique/) (anatomie industrielle 7 couches du backend), [*Coding agents*](../coding-agents/) (panorama produits dev), [*Agents computer-use*](../agents-computer-use/) (sous-régime on-behalf-of desktop), [*La fabrique d'un agent*](../fabrique-agent/) (organisation d'équipe et stades de maturité), [*Expériences narratives*](../narrative-experiences/) (généalogie des récits-données qui inspire le régime 3), [*MCP comme plateforme*](../mcp-plateforme/) (le standard côté tools, pendant d'AG-UI côté UI).
- L'écosystème AG-UI évolue rapidement. Les chiffres d'adoption, les statuts d'intégration (community / 1st-party / in progress) et les versions de hooks CopilotKit cités ici reflètent l'état au 18 mai 2026. Vérifier la documentation officielle avant adoption en production.
