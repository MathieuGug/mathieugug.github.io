...
6. **(Optional) A4 exec sum infographic** (fig-00) — if the dossier a 5+ messages-clés distillables et mérite d'être imprimé / partagé hors-écran (RDV, brief client, off-site), générer un SVG A4 portrait (`{YYYYMMDD}-00-exec-sum-a4.svg`) qui condense le rapport en 5 modèles mentaux. Injecté juste après la `<ul>` de la synthèse exécutive dans l'app. Spec complète (viewBox, marges, polices, layout) : `references/a4-exec-sum-infographic.md`. Skip si le dossier n'a que 3-4 messages forts ou si sa structure est intrinsèquement scrollytelling.
7. **Build the HTML companion app** following `references/companion-app.md` and `assets/app-template.html`. Inline all SVGs, all modal content, all tooltip definitions, all sources.
8. **(Optional) Build the slideshow companion** — if requested or warranted, build a 10-12 scene narrative slideshow following `references/slideshow.md`. **Read the "Quick start" section at the top of that file FIRST** — it lists 10 non-obvious adaptations (transparent SVG background, header wrapped in `<g class="svg-header">`, viewBox cropped to `y=155` per-SVG, title in topbar not stage, modal as right sidebar not centered, timeline vertical not bottom, `SCHEMAS` 2-level object, dynamic zoom refactor, editorial titles for punchlines, 3ʳᵈ hub card) that the user should not have to ask for. The slideshow reuses the same SVGs and `SCHEMAS` / `SOURCES` content as the long-form app — no new authoring.
9. **(Optional) Add quiz widgets** — if the article has clear *zones de confusion classique* where a survol-reader would walk away with a wrong intuition, identify 3-4 of those charnières conceptuelles and author a `.quiz-card` per charnière. **Default authoring path : JSON sidecar `quizzes.json` next to the app + one invocation of `assets/insert-quizzes.py`.** ~4× more token-efficient than inline HTML, idempotent, and the CSS + IIFE land automatically the first time the script runs on a quiz-less app. Schema and invocation in `references/quiz-authoring.md`; content rules (placement, mode mix, stabilo tile-par-ligne, accessibility) in `references/companion-app.md` § "Widgets quiz de compréhension". Skip this step entirely if the article doesn't have such charnières — quiz widgets are not a default.
10. **Package** the ZIP, move to `/mnt/user-data/outputs/`, and present with `present_files`.
...
- **Mobile-friendliness is non-negotiable.** Every shipped app must (a) ship `.panel-close` buttons inside `#toc` and `#sources` so a phone reader can close them, (b) keep header/main mobile typography inside the `@media (max-width: 1024px)` block (not a narrower `@640px`), (c) embed the defensive `html, body { max-width: 100vw } / body { overflow-x: hidden }` pair, and (d) ship the `main pre` + `main code` overflow rules even if the report has no `<pre>` yet. Full checklist in `references/mobile.md`. Panel-close pattern detail in `references/sidebars.md` § "Pattern `panel-close` (mobile)".
- **SEO + social previews are non-negotiable.** Every shipped HTML carries the full `<!-- og:start -->` … `<!-- og:end -->` block: Open Graph, Twitter Card `summary_large_image`, canonical URL, JSON-LD `Article`, plus a 1200×630 PNG `og.png` per dossier (NOT SVG — Twitter/LinkedIn/Slack reject SVG OG images). The host repo ships `tools/og-card.py` (PIL renderer) and `tools/seo_dossiers.py` (idempotent batch injector that parses `index.html` for the canonical metadata of each dossier). When this skill emits a new ZIP, the companion app's `<head>` already has the block populated via the templates' `{{CANONICAL_URL}}` / `{{OG_IMAGE_URL}}` / `{{KEYWORDS}}` / `{{PUBLISHED_DATE}}` markers; after dropping the ZIP into the host repo, run `tools/og-card.py` then `tools/seo_dossiers.py --only <slug>` to materialize the og.png and re-affirm the block. Full spec in `references/seo.md`.
...
| HTML app architecture (layout, modal system, tooltips, sources sidebar, JS) | `references/companion-app.md` |
| **Mobile-friendliness** — checklist 7 points (`overflow-x`, topbar responsive, typographie `@1024px`, SVG, panel-close, `<pre>`/`<code>`, `<table>`) | `references/mobile.md` |
| **Topbar 3 items** — pattern zones gauche/milieu/droite, constantes 56 px, ajustements sticky, vars CSS par format, pages hors scope | `references/topbar.md` |
| **Sidebars TOC + Sources** — sticky math `height` vs `max-height`, pattern `panel-close` mobile (CSS + HTML + JS), `sources-collapse-btn` `position: fixed`, format canonique `<li id="source-N">` bracketed + full URL | `references/sidebars.md` |
...
| **Optional A4 exec sum infographic** (fig-00 portrait, synthèse visuelle en 5 modèles mentaux, lisible en un coup d'œil, pensée pour partage RDV / pitch / impression A4) | `references/a4-exec-sum-infographic.md` |
...
2. **Author three fragment files** in the working directory:
   ...
   - `sources-fragment.html` — the `<li id="source-N">…</li>` entries (static
     HTML, bracketed `[N]` + full URL per `references/sidebars.md`).
...
- [ ] Fullscreen schema zoom works on each figure (hover reveals the ⛶ button); pressing Esc closes it. **`.zoom-btn` is at top-LEFT** (`left: 8px`), not top-RIGHT — the top-RIGHT corner is reserved for the schema sigil.
- [ ] **Schema sigil — MG monogram badge** — every `.figure` carries a `.schema-sigil` link in the top-RIGHT corner (`top: 72px` so it aligns with the schema title and stays visible in screenshots). The sigil is also cloned into the `#zoom-stage` (variant `.zoom-sigil`). Both link to `https://mathieugug.github.io/`. The injection IIFE adds the badge to any `figure.figure` that doesn't already have one — idempotent. See `references/companion-app.md` § 4b.
- [ ] Sources sidebar collapse/expand toggle works on desktop and persists across reloads (localStorage)
- [ ] **Sources sidebar stays pinned while scrolling** (`#sources` has `position: sticky; top: 56px; align-self: start; height: calc(100vh - 56px)` plus the transition, all on the SAME selector). No second `aside#sources { position: relative; … }` rule — it would override sticky by specificity. Verify on a long report: scroll past the first viewport, the sidebar must still be visible. Pattern and pitfall in `references/sidebars.md` § "Sticky math".
- [ ] **Sources are static HTML, not JS-generated.** `<ol id="sources-list">` contains one `<li id="source-N">` per source written directly in the body. Each `<li>` follows the bracketed + full URL format: `<span class="cite-num">[N]</span>` + citation text + `<br><a href="…full URL…">…full URL…</a>` + `<span class="accessed">`. Pattern canonique in `references/sidebars.md` § "Format canonique des sources".
...
- [ ] **SEO + social preview block** present in `<head>`, wrapped in `<!-- og:start -->` / `<!-- og:end -->` markers (idempotency anchor for `tools/seo_dossiers.py`). Must include: `og:type` (article for content, website for hubs), `og:title`, `og:description`, `og:image` (absolute https URL pointing to a 1200×630 **PNG**, never SVG), `og:image:width`/`height`, `og:url`, `og:site_name`, `og:locale=fr_FR`, `article:published_time` (ISO 8601), `article:author`, `twitter:card=summary_large_image`, `twitter:title`/`description`/`image`, `<link rel="canonical">`, `<meta name="author">`, and a JSON-LD `Article` (or `WebSite` for hubs) script. Full spec in `references/seo.md`.
- [ ] **og.png 1200×630 exists at `{slug}/og.png`** — generated by `tools/og-card.py` (PIL renderer using Cambria italic + Consolas, accent on one keyword from the title). Not SVG: Twitter/LinkedIn/Slack reject SVG OG images.
